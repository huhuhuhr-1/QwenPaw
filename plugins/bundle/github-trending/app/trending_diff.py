...
def find_new_entries(
    ...
    """对比两天 trending.items,返回只在 today 出现的 repo(返回副本避免外部修改源数据)。"""
    ...
    return [dict(item) for item in today_data.get("items", []) if item["full_name"] in new_names]


async def detect_and_record(today: str, yesterday: str, language: str = "all") -> int:
    """对比 today vs yesterday,新增的写 monitor_events。返回新增数。失败返回 0。"""
    try:
        ...
        logger.warning("Trending diff failed for %s/%s: %s", language, today, e)
        return 0
